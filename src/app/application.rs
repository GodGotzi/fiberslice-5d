...
use iced::core::time::Instant;
use iced::core::widget::operation;
use iced::core::window;
use iced::core::{Event, Size};
use iced::futures::futures;
use iced::futures::{Executor, Runtime, Subscription};
use iced::graphics::compositor::{self, Compositor};
use iced::runtime::clipboard;
use iced::runtime::program::Program;
use iced::runtime::user_interface::{self, UserInterface};
use iced::runtime::{Command, Debug};
use iced::style::application::{Appearance, StyleSheet};
use iced::{Clipboard, Error, Proxy, Settings};
    ...
    <A::Renderer as core::Renderer>::Theme: StyleSheet,
{
    ...
    let mut surface = compositor.create_surface(&window, physical_size.width, physical_size.height);
...
pub fn run_command<A, C, E>(
    ...
    <A::Renderer as core::Renderer>::Theme: StyleSheet,
{
    use iced::runtime::command;
    use iced::runtime::system;
        ...
        match action {
            ...
            command::Action::Window(action) => match action {
                ...
                window::Action::FetchSize(callback) => {
                    ...
                    proxy
                        .send_event(callback(Size::new(size.width, size.height)))
                        .expect("Send message to event loop")
                }
                ...
                window::Action::ChangeMode(mode) => {
                    ...
                    window.set_fullscreen(conversion::fullscreen(window.current_monitor(), mode));
                }
                ...
                system::Action::QueryInformation(_tag) => {
                    #[cfg(feature = "system")]
                    {
                        ...
                        let _ = std::thread::spawn(move || {
                            let information = iced::system::information(graphics_info);
